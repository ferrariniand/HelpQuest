...
import com.helpquest.core.database.mappers.toChatMessageEntity
...
interface ChatDao {
    ...
    @Query("SELECT * FROM chatentity ORDER BY lastActivityTimestamp DESC")
    @Transaction
        ...
        chats.forEach { chat ->
            chat.lastMessage?.let { lastMessage ->
                messageDao.upsertMessage(
                    lastMessage.toChatMessageEntity()
                )
    ...
    @Query("UPDATE chatentity SET lastActivityTimestamp = :timestamp WHERE chatId = :chatId")
